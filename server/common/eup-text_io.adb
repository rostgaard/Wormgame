 with Interfaces.C.Strings, POSIX.C;
...
--         POSIX.Permissions.Sticky         => 2#001_000_000_000#,
   ...
   procedure Get (File : in     File_Type;
      ...
      pragma Inline (Get);
      procedure Read_POSIX_Character is
         new POSIX.IO.Generic_Read (T => POSIX.POSIX_Character);
   begin
      Read_POSIX_Character (File => File,
                            Item => Item);
   end Get;
   ...
   begin
      for Index in Item'Range loop
         Get (File => File,
              Item => Item (Index));
         if Item (Index) = Ada.Characters.Latin_1.LF then
            Last := Index - 1;
            exit;
         end if;
      end loop;
